"""
CIF file manipulation utilities for ToBaCCo.

This module provides utilities for reading, writing, and reindexing CIF files.
"""
...
def iscoord(line: List[str]) -> bool:
    """
    Identify coordinate lines in CIF files.
    
    Args:
        line: Split line from CIF file
        
    Returns:
        True if line contains coordinate information
    """
    if len(line) < 5:
        return False
    
    return (re.sub('[^a-zA-Z]', '', line[0]) in PT and
            line[1] in PT and
            isfloat(line[2]) and
            isfloat(line[3]) and
            isfloat(line[4]))
...
def isbond(line: List[str]) -> bool:
    """
    Identify bonding lines in CIF files.
    
    Args:
        line: Split line from CIF file
        
    Returns:
        True if line contains bonding information
    """
    if len(line) < 5:
        return False
...
def reindex_cif(pathfile: str, charges: bool = False) -> None:
    """
    Reindex atoms in a CIF file with sequential numbering.
    
    This function reads a CIF file, renumbers all atoms sequentially,
    and updates all bond references accordingly. The output file is
    written with '_ri' suffix.
    
    Args:
        pathfile: Path to input CIF file
    ...
    with open(new_path, 'w') as out:
        ...
        for l in coords:
            ...
            out.write('{:7} {:>4} {:>15} {:>15} {:>15} {:>22} {:>15}'.format(
                name, elem, 
                "%.10f" % np.round(vec[0], 10), 
                "%.10f" % np.round(vec[1], 10), 
                "%.10f" % np.round(vec[2], 10),
                '0.00000  Uiso   1.00', 
                charge))
            out.write('\n')
        ...
        for l in bonds:
            out.write('{:7} {:>7} {:>5} {:>7} {:>3}'.format(
                l[0], l[1], "%.3f" % float(l[2]), l[3], l[4]))
            out.write('\n')


def nn(string: str) -> str:
