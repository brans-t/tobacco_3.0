...
import re
import os
from src.core.ciftemplate2graph import isvert
from itertools import combinations
import datetime
import networkx as nx
from src.utils.bbcif_properties import iscoord, isbond
from src.utils.paths import TEMPLATES_DIR, CHECK_CIFS_DIR, OUTPUT_CIFS_DIR
...
def write_check_cif(template, placed_nodes, placed_edges, g, sp, sc_unit_cell):
	...
	tpath = os.path.join(str(TEMPLATES_DIR), template)
	...
	cpath = os.path.join(str(CHECK_CIFS_DIR), str(g) + '_check_scaled_placed_' + template)
...
def write_cif(placed_all, fixed_bonds, scaled_params, sc_unit_cell, cifname, charges, wrap_coords=True):
	...
	opath = os.path.join(str(OUTPUT_CIFS_DIR), cifname)
