...
from src.utils.bbcif_properties import bb2array, X_vecs, bbbonds, bbcharges
from src.utils.paths import NODES_DIR, EDGES_DIR
from src.utils.svd_alignment import SVDSuperimposer
...
def place_nodes(nvecs, charges, ORIENTATION_DEPENDENT_NODES):
	...
	for n in nvecs:
		...
		bbxvec = np.array(X_vecs(cif, str(NODES_DIR), False))
		...
		bonds = bbbonds(cif, str(NODES_DIR))
...
def place_edges(evecs, charges, nnodes):
	...
	for e in evecs:
		...
		bbxvec  = np.array(X_vecs(cif, str(EDGES_DIR), False))
		nbbxvec = np.array([ll*(v / np.linalg.norm(v)) for v in bbxvec])

		min_dist,rot,tran = superimpose(nbbxvec,evec)

		all_bb = bb2array(cif, str(EDGES_DIR))
		all_coords = np.array([v[1] for v in all_bb])
		all_inds = np.array([v[0] for v in all_bb])
		chg, elem = bbcharges(cif, str(EDGES_DIR))
		all_names = [o + re.sub('[A-Za-z]','',p) for o,p in zip(elem,all_inds)]
		...
		bonds = bbbonds(cif, str(EDGES_DIR))
		anf = [str(elem_dict[n]) + str(ind_dict[n]) for n in all_inds]
